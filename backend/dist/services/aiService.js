...
export const aiService = {
    async generateJobOfferContext(interviewId) {
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: 'Tu es un expert en recrutement. Génère une offre d\'emploi fictive réaliste basée sur le profil du candidat. Réponds UNIQUEMENT en JSON valide avec cette structure exacte: {"jobTitle":"string","company":"string","description":"string","requirements":["string"],"niceToHave":["string"],"interviewFormat":"string","questions":["string"]}',
                    },
                    {
                        role: "user",
                        content: prompt,
...
`;
    ...
    async generateFeedback(question, answer, context) {
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            method: "POST",
            headers: {
                Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}`,
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: 'Tu es un expert en accompagnement aux entretiens. Analyse la réponse du candidat et fournis un feedback constructif. Réponds UNIQUEMENT en JSON avec: {"feedback":"string","score":number}',
                    },
                    {
                        role: "user",
                        content: `
