...
export default function InterviewContext() {
  ...
  const loadInterviewAndGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/interviews/${id}/context`, {
        method: 'POST',
    ...
    return (
      ...
      >
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-primary-dark mb-2">Génération en cours</h2>
    ...
    return (
      ...
      >
        <Card className="p-12 text-center">
          ...
          <h2 className="text-xl font-semibold text-primary-dark mb-2">Erreur de génération</h2>
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="mb-8"
      >
        <h1 className="text-3xl font-bold text-primary-dark mb-2">Offre d'emploi</h1>
        <p className="text-muted">Cette offre a été générée aléatoirement pour votre simulation</p>
      </motion.div>
      ...
      {context && (
        <div className="space-y-6">
          ...
          >
            <Card className="p-6 bg-gradient-to-r from-primary-dark to-primary text-white">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-2xl font-bold mb-1">{context.jobTitle}</h2>
          ...
          >
            <Card className="p-6">
              <h3 className="text-lg font-semibold text-primary-dark mb-4 flex items-center gap-2">
                <span className="material-icons text-purple-600">help</span>
                Questions à préparer
              </h3>
              <div className="grid md:grid-cols-2 gap-4">
                {context.questions.map((q, index) => (
                  <div key={index} className="bg-purple-50 border border-purple-100 rounded-lg p-4">
                    <p className="text-primary-dark font-medium mb-1">Question {index + 1}</p>
                    <p className="text-muted text-sm">{q}</p>
                  </div>
                ))}
              </div>
            </Card>
